...
void Emitter::UpdateEmitterPosition(float dt)
{
	dt *= 3;

	if (GetAsyncKeyState('W') & 0x8000) { emitterPosition.z += dt; }
	if (GetAsyncKeyState('S') & 0x8000) { emitterPosition.z -= dt; }
...
//	float speed = dt * 3;
//	if (cam->cameraMove == true)
//	{
//		if (GetAsyncKeyState('W') & 0x8000) { emitterPosition.z += speed; }
//		if (GetAsyncKeyState('S') & 0x8000) { emitterPosition.z -= speed; }
//		if (GetAsyncKeyState('A') & 0x8000) { emitterPosition.x -= speed; }
//		if (GetAsyncKeyState('D') & 0x8000) { emitterPosition.x += speed; }
//	}
